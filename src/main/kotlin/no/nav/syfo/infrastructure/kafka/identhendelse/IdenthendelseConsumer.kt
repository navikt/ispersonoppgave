...
class IdenthendelseConsumer(
    ...
    override val pollDurationInMillis: Long = 10000
    override fun pollAndProcessRecords(kafkaConsumer: KafkaConsumer<String, GenericRecord>) {
        runBlocking {
            try {
                val records = kafkaConsumer.poll(Duration.ofSeconds(pollDurationInMillis))
                if (records.count() > 0) {
    ...
    companion object {
        ...
        private val log: Logger = LoggerFactory.getLogger(IdenthendelseConsumer::class.java)
    }
}
