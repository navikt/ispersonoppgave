...
import no.nav.syfo.COUNT_PERSONOPPGAVEHENDELSE_DIALOGMELDING_SVAR_MOTTATT
...
class MeldingFraBehandlerService(
    ...
    internal fun processMeldingerFraBehandler(recordPairs: List<Pair<String, Melding>>) {
        ...
        database.connection.use { connection ->
            recordPairs.forEach { record ->
                ...
                log.info("Received meldingFraBehandler with key=${record.first}, uuid=${melding.referanseUuid} and parentRef=${melding.parentRef}")
    ...
    companion object {
        private val log: Logger = LoggerFactory.getLogger(MeldingFraBehandlerService::class.java)
    }
}
